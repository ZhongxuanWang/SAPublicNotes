...
def signup_post():
    password = request.form.get('password')
    password_confirmed = request.form.get('password_conf')
    if password_confirmed != password:
        return render_template('auth/signup.html', err_msg='Passwords are not the same')
    year = int(request.form.get('year'))
    if year > 2024 or year < 2021:
        return render_template('auth/signup.html', err_msg='Only accessible to current upper school students.')
    ...
    new_user = User(email=email, name=name, password=generate_password_hash(password, method='sha256'), role=1,
                    grad_year=year)
